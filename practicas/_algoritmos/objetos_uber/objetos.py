...
class Vehiculo:
    ...
    def mover(self, matriz:list[list]):
        if self.cliente != None:  # solo se mueve si tiene un objetivo: buscar o transportar clientes
            if self.ocupado:
                ...
                self.avanzar(matriz, x_delta, y_delta)
    ...
    def avanzar(self, matriz:list[list], x_delta:int, y_delta:int):
        func.borrar_en_ciudad(matriz, self)
        ...
        func.posicionar_en_ciudad(matriz, self)
        self.coste_viaje += 1
